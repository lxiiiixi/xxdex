import { CHAIN_IDS } from "@/config/utils/wagmi";
...
const AddLiquidityPage = () => {
    ...
    const preferFarmType = useMemo(() => {
        ...
        const hasV3Farm = farmV3Public?.farmsWithPrice.find(
            farm =>
                farm.multiplier !== "0X" &&
                ((farm.token.equals(currencyA.wrapped) &&
                    farm.quoteToken.equals(currencyB.wrapped)) ||
                    (farm.quoteToken.equals(currencyA.wrapped) &&
                        farm.token.equals(currencyB.wrapped)))
        );
        if (hasV3Farm)
            return {
